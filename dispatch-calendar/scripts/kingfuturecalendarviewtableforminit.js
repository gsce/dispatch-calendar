...
function jobFormDateTableUpdate(rowObject,formTable,calendarObject) {
	...
	if (e.target.parentNode.cells[0].className === 'jobformdatetabledaynoterow'){
		var rowNumber = e.target.parentNode.rowIndex - 1;
	   	...
	   	document.getElementsByName(formTable.name+'drivernumber')[0].value = document.getElementById('jobformdatetable').rows[rowNumber].cells[8].textContent.trim();
	   	document.getElementsByName(formTable.name+'namesofmen')[0].value = document.getElementById('jobformdatetable').rows[rowNumber].cells[9].textContent.trim();
	    ...
	    document.getElementsByName(formTable.name+'drivernumber')[0].value = e.target.parentNode.cells[8].textContent.trim();
	    document.getElementsByName(formTable.name+'namesofmen')[0].value = e.target.parentNode.cells[9].textContent.trim();
...
function jobFormDateTableUpdateDateAdd(formTable,calendarObject,e) {
	...
	document.getElementsByName(formTable.name+'drivernumber')[0].value = document.getElementById('jobformdatetable').rows[i].cells[8].textContent.trim();
	document.getElementsByName(formTable.name+'namesofmen')[0].value = document.getElementById('jobformdatetable').rows[i].cells[9].textContent.trim();
...
function jobDateFormDone(formTable) {
    ...
    //parent.document.forms['jobform'].submit();
	return;
}

function jobDateFormAddAnotherDate(formTable) {
    formTable.name = 'jobdateform';
	alert('add');
    document.getElementById('whereupdated').value = 'popupcalendar';
	parent.document.getElementById('jobformbottomboxdiv').style.left = '0%';
...
function init() {
    ...
    // mouseClickOnJobDateFormCancelButton.addEventListener('click', function() {refreshPage(calendarObject)}, false);
	...
	var mouseClickOnJobDateFormDateButton = document.getElementById('jobdateformdatebutton');
    mouseClickOnJobDateFormDateButton.addEventListener('click', function() {
        ...
        locationAction = 'http://localhost/king/kingpopupcalendar.php?calendardate='+calendarObject.date+'&calendaremployeeid='+calendarObject.employeeId+'&calendaremployeename='+calendarObject.employeeName+'&calendarid='+calendarObject.calendarId;
	...
	for (var dateEvent = 0; dateEvent < 120; dateEvent++) {
